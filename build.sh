...
if [[ $INSTALL_NET_WIFI == 1 ]]; then
  PKGS="$PKGS iw wpasupplicant wireless-regdb"
fi
...
if [[ $INSTALL_MEDIAUTILS == 1 ]]; then
  PKGS="$PKGS ffmpeg vlc vlc-plugin-base"
fi
...
if [[ $INSTALL_GPM == 1 ]]; then
  (cd $ROOTFS/etc/; ln -sfr init.d/gpm rc1.d/S90gpm) 
fi
...
mkdir -p $HOMEFS/$USER
cp misc-config/bashrc $HOMEFS/$USER/.bashrc
cp misc-config/bash_profile $HOMEFS/$USER/.bash_profile
(cd $HOMEFS/$USER; ln -sfr /mnt mnt)
